...
def load_full_job(un_job_stub: UNJobStub):
    return UNJob(un_job_stub.un_jobnet_id,
                 un_job_stub.title,
...
def _fetch_job_description(job_stub: UNJobStub) -> str:
    ...
    if job_stub.organization_short_name == 'IOM' and "View on Organization Website" in page.text:
...
def load_new_job_stubs_at_url(url: str, until_page: int, seen_jobs: set[str]) -> list[UNJob]:
    ...
    for page_number in range(1, until_page + 1):
        ...
        new_job_stubs = [job for job in new_job_stubs if job.un_jobnet_id not in seen_jobs]
